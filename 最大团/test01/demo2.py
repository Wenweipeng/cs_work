...
class MainDrawWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.tool_new_file = QAction(
            QIcon("./images/file-add.png"), '新建图', self)
        self.tool_open_file = QAction(
            QIcon("./images/file-open.png"), '打开矩阵', self)
        self.tool_save_file = QAction(QIcon("./images/save.png"), '保存矩阵', self)

        self.tool_add_circular = QAction(
            QIcon("./images/add-circle.png"), '添加节点', self)
        ...
        self.group_box_btn_grid_layout.addWidget(
            self.only_triu_matrix_checkbox, 0, 0, 1, 1)
        self.group_box_btn_grid_layout.addWidget(
            self.btn_clear_matrix, 1, 0, 1, 1)
        self.group_box_btn_grid_layout.addWidget(
            self.btn_add_circular, 1, 1, 1, 1)
        self.group_box_btn_grid_layout.addWidget(self.btn_commit, 1, 2, 1, 1)
        self.group_box_btn_grid_layout.addWidget(
            self.btn_cancel_all, 1, 3, 1, 1)
        ...
        self.group_box_btn_grid_layout.addWidget(
            self.label_generate_matrix_hint, 2, 0, 1, 1)
        self.group_box_btn_grid_layout.addWidget(
            self.label_matrix_points, 3, 0, 1, 1)
        self.group_box_btn_grid_layout.addWidget(self.edit_points, 3, 1, 1, 1)
        self.group_box_btn_grid_layout.addWidget(
            self.label_matrix_seed, 3, 2, 1, 1)
        self.group_box_btn_grid_layout.addWidget(self.edit_seed, 3, 3, 1, 1)
        self.group_box_btn_grid_layout.addWidget(
            self.label_matrix_edges, 4, 0, 1, 1)
        ...
        self.combobox_mc_algorithm.addItems(
            ['random walk algorithm', 'mc base on bk'])
        self.group_box_mc_grid_layout.addWidget(
            self.combobox_mc_algorithm, 0, 0, 1, 2)
        ...
        self.tool_add_circular.triggered.connect(
            lambda: self.func_btn_check_able_check(self.tool_add_circular))
        # noinspection PyTypeChecker
        self.tool_add_edge.triggered.connect(
            lambda: self.func_btn_check_able_check(self.tool_add_edge))

        # 点数输入框
        self.edit_points.textChanged.connect(self.func_edit_points_changed)
        # 只显示对角阵复选框
        self.only_triu_matrix_checkbox.clicked.connect(
            self.func_checkbox_changed)
        ...
        self.combobox_mc_algorithm.currentIndexChanged.connect(
            self.func_combobox_change)
        # </editor-fold>
    ...
    def func_open_matrix(self):
        ...
        if fileName_choose != '':
            ...
            if os.path.exists(fileName_choose + '.params'):
                ...
                self.edit_points.setText(
                    params_file.readline().split('=')[1].strip())
                self.edit_edges.setText(
                    params_file.readline().split('=')[1].strip())
            ...
            if _m is not None:
                if filetype.find('npy') > 0:
                    ...
                    with open(fileName_choose + '.params', 'w') as f:
                        ...
                        f.write(
                            f'random_seed={None if self.edit_seed.text() == "" else self.edit_seed.text()}')
    ...
    def func_edit_points_changed(self):
        ...
        if (_p := self.edit_points.text()).isdecimal():
            ...
            self.edit_edges.setPlaceholderText(
                str(f'max edge is {self.ALLOW_INPUT_MAX_EDGES}'))
    ...
    def func_generate_matrix(self):
        ...
        if _points.isdecimal() and _edges.isdecimal():
            _points = int(_points)
            _edges = int(_edges)
            self.generate_matrix_by_params(
                _points, _edges, None if _seed == '' else _seed)
            # 生成完了备份
            self.BACKUP_MATRIX_TRIU = self.MATRIX_TRIU.copy()
            self.render_matrix(self.MATRIX_TRIU)
            self.statusBar().showMessage(
                f'已使用随机参数：点={_points}，边={_edges}，随机种子={"无" if _seed == "" else _seed}')
        else:
            # 状态栏或对话框提示错误
    ...
    def func_add_circular_in_model(self):
        ...
        self.table_model.setHorizontalHeaderItem(
            _rows, QStandardItem(str(_rows)))
        self.table_model.setVerticalHeaderItem(
            _rows, QStandardItem(str(_rows)))
        self.table_model.horizontalHeaderItem(
            _rows).setFont(self.font_table_header)
        self.table_model.verticalHeaderItem(
            _rows).setFont(self.font_table_header)
    ...
    def func_mc_play(self):
        ...
        for _each_solution in self.solution_all:
            ...
            for _each_point in _each_solution:
                ...
                self.paint_single_point(self.pix, str(
                    _each_point), color='red', broad=4)
                ...
                time.sleep(1)
            # 打印当前最大团结果 红色
            self.paint_single_line(self.pix, str(
                self.clique_list[-1]), str(self.clique_list[0]), color='red', broad=4)
            self.q_label.setPixmap(self.pix)
            # 打印log
        ...
        self.text_log_edit.append(
            f'最大团：<font color="red">{", ".join(self.solution_max)}</font><br>')
        # 恢复按钮
    ...
    def func_mc_next(self):
        if self.combobox_mc_algorithm.currentText() == 'mc base on bk':
            ...
            if not self.MC_STEP_MODEL:
                ...
                self.backupPix = self.pix.copy()
                self.paint_single_point(self.pix, str(
                    self.clique_list[0]), color='red', broad=4)
                ...
                if self.mc_step_current_point > len(self.clique_list):
                    ...
                    self.text_log_edit.append(
                        f'极大团：{", ".join([str(_) for _ in self.clique_list])}')
                    # 若当前极大团遍历完
            ...
            if not self.MC_STEP_MODEL:
                ...
                for _index, node in enumerate(similar_nodes):
                    if _index == 0:
                        self.paint_single_point(
                            self.pix, '0', color='red', broad=4)
                    else:
                        self.paint_single_point(
                            self.pix, str(node), color='green', broad=4)
                self.text_log_edit.append(
                    f'for node <font color="red">0</font>, '
                    f'the similar <font color="green">{", ".join([str(_) for _ in similar_nodes[1:].tolist()])}</font>')
            else:
                self.mc_step_current_point += 1
                self.mc_step_current_point = self.mc_step_current_point % len(
                    self.MATRIX_DIAGONAL)
                if self.mc_step_current_point < len(self.models):
                    self.pix = self.backupPix.copy()
                    similar_nodes = self.get_similar_nodes(
                        self.mc_step_current_point, 5, self.models[0])

                    for _index, node in enumerate(similar_nodes):
                        if _index == 0:
                            self.paint_single_point(
                                self.pix, str(node), color='red', broad=4)
                        else:
                            self.paint_single_point(
                                self.pix, str(node), color='green', broad=4)
                    self.text_log_edit.append(
    ...
    def func_mc_previous(self):
        if self.combobox_mc_algorithm.currentText() == 'mc base on bk':
            ...
            for _i in range(self.mc_step_current_point if self.mc_step_current_point == len(
                ...
                self.paint_single_point(self.pix, str(
                    self.clique_list[_i]), color='red', broad=4)
                if _i > 0:
    ...
    def render_matrix(self, aj_matrix: np.array = None):
        ...
        self.line_list.clear()
        # 加载节点坐标，边信息
        temp_pos_dict, self.line_list = get_position(
            self.PIX_WIDTH - 100, self.PIX_HEIGHT - 100, aj_matrix)
    ...
    def paint_single_point(self, pix: QPixmap, p_id: str, color: str = 'black', broad: int = 2):
        pp = QPainter(pix)
        pp.begin(self)
        pen_color = {'black': Qt.black, 'red': Qt.red,
                     'green': QColor(112, 173, 71)}.get(color)
        pp.setPen(QPen(pen_color, broad, Qt.SolidLine))
        pp.setFont(self.font_num)
        c = self.circular_pos_dict[p_id]
        pp.drawEllipse(c, self.CIRCULAR_RADIUS, self.CIRCULAR_RADIUS)
        if len(p_id) < 2:
            pp.drawText(round(c.x() - self.CIRCULAR_RADIUS / 3),
                        round(c.y() + self.CIRCULAR_RADIUS / 2), p_id)
        else:
            pp.drawText(round(c.x() - self.CIRCULAR_RADIUS / 1.3),
                        round(c.y() + self.CIRCULAR_RADIUS / 2), p_id)
        pp.end()
    ...
    def paintEvent(self, event):
        if 'circular' == self.draw_tag:
            ...
            pp.begin(self)
            pp.setPen(QPen(Qt.black, 2, Qt.SolidLine))
            pp.drawEllipse(self.lastPoint, self.CIRCULAR_RADIUS,
                           self.CIRCULAR_RADIUS)
            pp.end()
            ...
            if self.isDrawing:
                ...
                pp.setPen(QPen(Qt.black, 2, Qt.SolidLine))
                pp.drawEllipse(
                    self.lastPoint, self.CIRCULAR_RADIUS, self.CIRCULAR_RADIUS)
                pp.end()
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.LeftButton:
            self.lastPoint = QPoint(
                event.pos().x() - self.MOUSE_BIAS_X, event.pos().y() - self.MOUSE_BIAS_Y)
            ...
            if self.tool_add_edge.isChecked():
                ...
                if self.tool_add_circular.isChecked():
                    ...
                    if not self.__is_new_circular_exist(self.lastPoint):
                        self.circular_pos_dict[str(
                            self.MAX_POINTS_NUM)] = self.lastPoint
